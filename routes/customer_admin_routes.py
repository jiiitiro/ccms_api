...
from models import CustomerAdminLogin
...
@customer_admin_api.get("/customer/admin/all")
def get_all_data():
...
@customer_admin_api.get("/customer/admin/<int:login_id>")
...
@customer_admin_api.post("/customer/admin/register")
def register():
    api_key_header = request.headers.get("x-api-key")
    if api_key_header == API_KEY:
        try:
            ...
            existing_customer = CustomerAdminLogin.query.filter_by(email=request.form.get("email")).first()
            ...
            body = (f"Click the following link to confirm your email: "
                    f"{BASE_URL}/customer/admin/confirm-email/{token}")
...
# Confirm Email
@customer_admin_api.get("/customer/admin/confirm-email/<token>")
...
@customer_admin_api.post("/customer/admin/login")
def login_admin():
    api_key_header = request.headers.get("x-api-key")
    if api_key_header == API_KEY:
        try:
            user = CustomerAdminLogin.query.filter(CustomerAdminLogin.email == request.form.get("email")).first()
...
@customer_admin_api.patch("/customer/admin/<int:login_id>")
def update_user(login_id):
    api_key_header = request.headers.get("x-api-key")
    if api_key_header == API_KEY:
        try:
            user_to_update = CustomerAdminLogin.query.filter_by(login_id=login_id).first()
            ...
            if user_to_update:
                ...
                return jsonify(success={"message": "User data updated successfully",
                                        "customer_data": updated_user_data}), 200
            else:
                return jsonify(error={"message": "user not found"}), 404
...
@customer_admin_api.delete("/customer/admin/<int:login_id>")
...
def send_reset_email(email, reset_token, name):
    ...
    body = f"""
            ...
            <body>
                <div class="container">
                    <h1>Dear {name},</h1>
                    <p>Click the following link to reset your password:  <a href="{BASE_URL}/customer/admin/reset-password/{reset_token}">Reset Password</a></p>
...
@customer_admin_api.post("/customer/admin/reset-password/")
def user_forgot_password():
    api_key_header = request.headers.get("x-api-key")
    if api_key_header == API_KEY:
        try:
            email = request.form.get("email")
            existing_user = CustomerAdminLogin.query.filter_by(email=email).first()
...
@customer_admin_api.route("/customer/admin/reset-password/<token>", methods=['GET', 'POST'])
...
@customer_admin_api.put("/customer/admin/change-password/<int:login_id>")
def user_change_password(login_id):
    api_key_header = request.headers.get("x-api-key")
    if api_key_header == API_KEY:
        try:
            # Assuming you have a CustomerLogin model
            user_to_change_pass = CustomerAdminLogin.query.filter_by(login_id=login_id).first()
