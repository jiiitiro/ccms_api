...
@billing_admin_api.get("/billing/admin/all")
def get_all_data():
...
@billing_admin_api.get("/billing/admin/<int:login_id>")
...
@billing_admin_api.post("/billing/admin/register")
def register():
    api_key_header = request.headers.get("x-api-key")
    if api_key_header == API_KEY:
        try:
            ...
            body = (f"Click the following link to confirm your email: "
                    f"{BASE_URL}/billing/admin/confirm-email/{token}")
...
# Confirm Email
@billing_admin_api.get("/billing/admin/confirm-email/<token>")
...
@billing_admin_api.post("/billing/admin/login")
def login_admin():
    api_key_header = request.headers.get("x-api-key")
    if api_key_header == API_KEY:
        try:
...
@billing_admin_api.patch("/billing/admin/<int:login_id>")
...
@billing_admin_api.delete("/billing/admin/<int:login_id>")
...
@billing_admin_api.post("/billing/admin/reset-password")
...
@billing_admin_api.put("/billing/admin/change-password/<int:login_id>")
...
@billing_admin_api.post("/billing/admin/logout/<int:login_id>")
def admin_payroll_logout(login_id):
    try:
        ...
        query_data = BillingAdminLogin.query.filter_by(login_id=login_id).first()

        if query_data is None:
            return jsonify(error={"message": "Login id not found."}), 404

        log_activity(BillingAdminActivityLogs, login_id=query_data.login_id,
